...
- Journey card cleanup: removed the small `dash-label` captions from the right-side journey image cards and removed the active-card child `gsap.fromTo` fade/stagger that caused janky image blinking. Active state still changes card emphasis and keeps the subtle step-number motion. Verification: `npm run format:check`, `npm run build`, `git diff --check`, and browser QA passed; `.stage-card .dash-label` count is 0, card titles remain, no small caption text remains inside cards, no horizontal overflow, and console has 0 errors/warnings.
